...
class CryptoTradingEnv(gym.Env):
    ...
    def __init__(self, df: DataFrame, window_length: int, initial_capital: float, transaction_cost_pct: float,
                 reward_scale: float, print_verbosity: int, memory_dir: str):
        self._df = df
    ...
    def _sell_token(self) -> None:
        ohlcv = self._get_state_ohlcv()

        sell_token_count = int(self._balance.token)
    ...
    def _buy_token(self) -> None:
        ohlcv = self._get_state_ohlcv()

        buy_token_count = int((self._balance.capital / (1 + self._transaction_cost_pct)) // ohlcv.close)
    ...
    def step(self, action: int) -> (State, Reward, Done, Info):
    ...
    def _save_action_memory(self) -> pd.DataFrame:
        ...
        scalar_action = [a for a in self._memory.action]
